...
public class MessageCompression {
	public byte[] compress(byte[] data) {
		...
		byte[] dataToCompress = ByteBuffer.allocate(data.length - 2)
				.put(data, 2, data.length - 2).array();
		...
		if (length < dataToCompress.length) {
			return ByteBuffer.allocate(length + 3).put(data[0]).put(data[1])
					.put(result, 0, length).put((byte) 1).array();
		}
		return ByteBuffer.allocate(data.length + 1).put(data).put((byte) 0)
				.array();
	}
	...
	public byte[] decompress(byte[] data) {
		...
		if (data[(data.length - 1)] == 0) {
			return ByteBuffer.allocate(data.length - 1)
					.put(data, 0, data.length - 1).array();
		}
		int startingIndex = (data[0] > 0) && (data[0] < 16) ? 1 : 2;
